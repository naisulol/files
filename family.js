...
class FamilyWebApp extends React.Component {
  ...
  initializeNetworkData = (data) => {
    ...
    objects.forEach((entry, i) => {
      ...
      if (kuya_id !== "") {
        // Some genbod have the exception of 2 Ate's so in the CSV file, add an X after the ID to indicate that the person is an Ate, not Kuya
        if (kuya_id.length === 4) {
          let new_kuya_id = Number(kuya_id.substring(0, 3));
